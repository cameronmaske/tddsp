...
from tddsp import core, effects, processors, synths
import numpy as np
import torch as th
...
class TestProcessorGroup:
    def setup_method(self):
        ...
        def rand_signal(channels):
            return th.randn(self.n_batch, channels, self.n_frames)
        ...
        self.nn_outputs = {
            ...
            "target_audio": th.randn(self.n_batch, self.n_time),
        }
    ...
    def _check_tensor_outputs(self, strings_to_check, outputs):
        for tensor_string in strings_to_check:
            ...
            assert th.is_tensor(tensor)

    def test_dag_construction(self):
        """Tests if DAG is built properly and runs.
    """
        processor_group = processors.ProcessorGroup(
            dag=self.dag, name="processor_group"
        )
...
class TestAdd:
    def test_output_is_correct(self):
        ...
        x = th.zeros((2, 3), dtype=th.float32) + 1.0
        y = th.zeros((2, 3), dtype=th.float32) + 2.0

        output = processor(x, y)

        expected = th.zeros((2, 3), dtype=th.float32) + 3.0
...
class TestMix:
    def test_output_shape_is_correct(self):
        ...
        x1 = th.zeros((2, 3, 100), dtype=th.float32) + 1.0
        x2 = th.zeros((2, 3, 100), dtype=th.float32) + 2.0
        mix_level = (
            th.zeros((2, 1, 100), dtype=th.float32) + 0.1
        )  # will be passed to sigmoid

        output = processor(x1, x2, mix_level)

        assert [2, 3, 100] == list(output.shape)
