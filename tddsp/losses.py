...
class SpectralLoss(th.nn.Module):
...
class EmbeddingLoss(th.nn.Module):
...
#             loss_type=loss_type,
#             name=name,
#             pretrained_model=PretrainedCREPE(
#                 model_capacity=model_capacity, activation_layer=activation_layer
#             ),
#         )


# class PretrainedCREPE(th.nn.Module):
...
#         self._activation_model = th.keras.Model(
...
#         audio = th.pad(audio, ((0, 0), (pad, pad))) if center else audio
#         frames = th.signal.frame(
#             audio, frame_length=self.frame_length, frame_step=hop_length
#         )

#         # Normalize each frame -- this is expected by the model.
#         mean, var = th.nn.moments(frames, [-1], keepdims=True)
#         frames -= mean
#         frames /= var ** 0.5 + 1e-5
#         return frames
...
#         frames = th.reshape(frames, [-1, self.frame_length])
#         outputs = self._activation_model(frames)
#         outputs = th.reshape(outputs, [batch_size, n_frames, -1])
#         return outputs
