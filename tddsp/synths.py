...
from tddsp import core, processors
import torch as th
...
class Additive(processors.Processor):
    ...
    def get_controls(self, amplitudes, harmonic_distribution, f0_hz):
        """Convert network output tensors into a dictionary of synthesizer
        controls.
    ...
    def get_signal(self, amplitudes, harmonic_distribution, f0_hz):
        ...
        Args:
            amplitudes: Amplitude tensor of shape [batch_size, n_frames, 1].
                Expects float32 that is strictly positive.
            harmonic_distribution: Tensor of shape
                [batch_size, n_frames, n_harmonics]. Expects float32 that is
                strictly positive and normalized in the last dimension.
            f0_hz: The fundamental frequency in Hertz. Tensor of shape [batch_size,
                n_frames, 1].

        Returns:
            signal: A tensor of harmonic waves of shape [batch_size, n_samples].
        """
...
class FilteredNoise(processors.Processor):
    ...
    def get_signal(self, magnitudes):
        ...
        Returns:
            signal: A tensor of harmonic waves of shape [batch_size, n_samples, 1].
        """
        batch_size = magnitudes.shape[0]
        signal = th.empty(batch_size, self.n_samples).uniform_(-1, 1)
...
class Wavetable(processors.Processor):
    ...
    def get_controls(self, amplitudes, wavetables, f0_hz):
        """Convert network output tensors into a dictionary of synthesizer
        controls.
    ...
    def get_signal(self, amplitudes, wavetables, f0_hz):
        ...
        Args:
            amplitudes: Amplitude tensor of shape [batch_size, n_frames, 1]. Expects
                float32 that is strictly positive.
            wavetables: Tensor of shape [batch_size, n_frames, n_wavetable].
            f0_hz: The fundamental frequency in Hertz. Tensor of shape [batch_size,
                n_frames, 1].

        Returns:
            signal: A tensor of of shape [batch_size, n_samples].
        """
        wavetables = core.resample(wavetables, self.n_samples)
        signal = core.wavetable_synthesis(
            amplitudes=amplitudes,
            wavetables=wavetables,
            frequencies=f0_hz,
            n_samples=self.n_samples,
